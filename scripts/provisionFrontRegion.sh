...
## ARGUMENT INPUT            ##
###############################
usage() { echo "Usage: provisionFront.sh <unique>" 1>&2; exit 1; }
...
FRONT_SUBNET=$(az network vnet subnet show --name front \
  --resource-group ${UNIQUE} \
  --vnet-name ${UNIQUE}-vnet \
  --query id -otsv)
DMZ_SUBNET=$(az network vnet subnet show --name dmz \
  --resource-group ${UNIQUE} \
  --vnet-name ${UNIQUE}-vnet \
  --query id -otsv)
...
az group deployment create \
  ...
  --parameters unique=${UNIQUE} sasToken=?$TOKEN \
  --parameters keyVaultId=${KEYVAULT} subnetIdDMZ=${DMZ_SUBNET} subnetIdFront=${FRONT_SUBNET}\
